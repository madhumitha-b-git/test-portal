...
# This script forces pip to download Linux-compatible binaries instead of Windows binaries

$PROJECT_DIR = Split-Path -Parent $MyInvocation.MyCommand.Path
$BUILD_DIR = "$env:TEMP\lambda-build-nodocker-$([System.IO.Path]::GetRandomFileName())"
...
Write-Host "Building Lambda package locally (No Docker)..." -ForegroundColor Cyan
...
# Install dependencies using pip cross-compilation for Amazon Linux (manylinux2014_x86_64)
Write-Host "Downloading Linux-compatible Python packages directly..." -ForegroundColor Cyan
$REQ_FILE = "$PROJECT_DIR\requirements-lambda.txt"

# Force pip to download the Linux (manylinux) versions instead of Windows versions
pip install -r "$REQ_FILE" --target "$BUILD_DIR\package" --platform manylinux2014_x86_64 --implementation cp --python-version 3.10 --abi cp310 --only-binary=:all:
